...
#domain name same as user name
DOMAIN_NAME=$NOME_UTENTE
...
"load")
	...
	fab -u $NOME_UTENTE -H $HOST_REMOTO create_digital_files_dir:$NOME_UTENTE,$REPO_NAME -p $USER_PASS || exit 1
...
"setup")
    ...
    fab -u $NOME_UTENTE -H $HOST_REMOTO nginx_uwsgi_setup_config:$PROJECT_NAME,$REPO_NAME,$DOMAIN_NAME -p $USER_PASS || exit 1
    exit 0
    ;;
"check")
